...
class FilterForm extends React.Component {
    ...
    //动态表单
    dongtaibiaodan = () => {
        axios.get("http://118.31.45.118:8888/lvyou-ssm1/ceshisss",
            {
                withCredentials: true
            }
        ).then(res => {
                let form = this.state.options;
                form = res.data.options
                this.setState(
                    {
                        options: form,
                    },
    ...
    initFormList = () => {
        ...
        if (formList && formList.length > 0) {
            formList.forEach((item, i) => {
                ...
                else if (item.type == '年份') {
                    const Date = <FormItem label={label} key={field}>
                        {
                            getFieldDecorator([field])(
                                <MonthPicker showTime={true}  picker="year" placeholder={placeholder} format="YYYY-MM" />
                            )
                        }
                    </FormItem>;
                    formItemList.push(Date)
