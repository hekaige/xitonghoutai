...
let list
export default class qshipinqqdyisjhzxxb extends React.Component {
    state = {
        orderInfo: {},
        list: [],
    }
    ...
    toExcel() {
        const data = [
            ['视频名称', '播放量', '', '', '', '', '', '','','','','会员观看时长（小时）',  '', '', '', '', '', '','','','','热度', '', '', '', '', '', '','','','','播放指数' ,'', '', '', '', '', '','','','',],
            ['', '爱奇艺', '优酷', '腾讯', '小米', '海信','创维', '咪咕','B站','长虹','华为', '爱奇艺', '优酷', '腾讯',  '小米', '创维','海信','咪咕','B站','长虹','华为' , '爱奇艺', '优酷', '腾讯',  '小米','创维', '海信','咪咕','B站','长虹','华为' , '爱奇艺', '优酷', '腾讯',  '小米', '创维','海信','咪咕','B站','长虹','华为'  ]
        ]
        ...
        XlSX.writeFile(wb, `全视频全渠道运营数据汇总（详细版）.xlsx`)
    }
    componentDidMount() {
        this.requestList();
    }
    ...
    chaxun = (params) => {
        ...
        params.time2= Y2+M2
        axios.get("http://118.31.45.118:8888/lvyou-ssm1/qspqqdyyhuizongxx?time1=" + params.time1 + "&time2=" + params.time2,
        {
    ...
    //默认请求接口数据
    requestList = () => {
        axios.get('http://118.31.45.118:8888/lvyou-ssm1/qspqqdyyhuizongxx',
        {
            withCredentials: true
        }
        ).then((res) => {
