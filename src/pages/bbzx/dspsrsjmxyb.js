...
let list
export default class dspsrsjmxyb extends React.Component {
    state = {
        orderInfo: {},
        list: [],
    }
    ...
    //导出
    toExcel() {
        const data = [["日期", "总计（元）", "广告收入（元）", "会员收入（元）"]]
        ...
        XlSX.writeFile(wb, `单视频收入数据明细月表.xlsx`)
    }
    componentDidMount() {
        this.requestList();
    }
    ...
    chaxun = (params) => {
        ...
        params.time2= Y2+M2
        axios.get("http://118.31.45.118:8888/lvyou-ssm1/dspsrsjmxyuebiao?name=" + params.name[0] + "&qdao=" + params.name[1] + "&time1=" + params.time1 + "&time2=" + params.time2,
        {
    ...
    //默认请求接口数据
    requestList = () => {
        axios.get('http://118.31.45.118:8888/lvyou-ssm1/dspsrsjmxyuebiao',
        {
            withCredentials: true
        }
        ).then((res) => {
    ...
    //图表
    openOrderDetail = () => {
        window.open(`/#/common/order/detail5/：orderId5`, '_blank')
    }
    render() {
        const columns = [
            {
