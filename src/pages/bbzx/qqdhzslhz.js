...
let list;
export default class qqdhzslhz extends React.Component {
    state = {
        orderInfo: {},
        list: [],
    }
    ...
    //导出
    toExcel() {
        const data = [["序号", "渠道分类", "渠道名称", "合作起始时间", "总合作数量", "会员合作数量", "广告合作数量", "独代合作数量", "备注"]]
        ...
        XlSX.writeFile(wb, `全渠道合作数量汇总.xlsx`)
    }
    componentDidMount() {
        this.requestList();
    }
    ...
    chaxun = (params) => {
        ...
        params.time2= Y2+M2
        axios.get( "http://118.31.45.118:8888/lvyou-ssm1/qqdhzslhuizong?time1=" + params.time1 + "&time2=" + params.time2,
        {
    ...
    //默认请求接口数据
    requestList = () => {
        axios.get('http://118.31.45.118:8888/lvyou-ssm1/qqdhzslhuizong',
        {
            withCredentials: true
        }
        ).then((res) => {
