...
let list
export default class qqdsrhz extends React.Component {
    state = {
        orderInfo: {},
        list: [],
    }
    ...
    //导出
    toExcel() {
        const data = [["序号", "渠道分类", "渠道名称", "合作起始时间", "合作总收入", "会员收入", "广告收入", "独代收入", "备注"]]
        ...
        XlSX.writeFile(wb, `全渠道收入汇总.xlsx`)
    }
    componentDidMount() {
        this.requestList();
    }
    ...
    chaxun = (params) => {
        ...
        params.time2= Y2+M2
        axios.get("http://118.31.45.118:8888/lvyou-ssm1/qqdsrhuizong?time1=" + params.time1 + "&time2=" + params.time2,
        {
    ...
    //默认请求接口数据
    requestList = () => {
        axios.get('http://118.31.45.118:8888/lvyou-ssm1/qqdsrhuizong',
        {
            withCredentials: true
        }
        ).then((res) => {
    ...
    openOrderDetail = (params) => {
        window.open(`/#/common/order/detail7/：orderId7`, '_blank')
    }
    render() {
        const columns = [
            {
