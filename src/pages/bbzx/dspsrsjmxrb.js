...
let list
export default class dspsrsjmxrb extends React.Component {
    state = {
        orderInfo: {},
        list: [],
    }
    ...
    toExcel() {
        ...
        XlSX.writeFile(wb, `单视频收入数据明细日表.xlsx`)
    }
    componentDidMount() {
        this.requestList();
    }
    ...
    chaxun = (params) => {
        ...
        axios.get("http://118.31.45.118:8888/lvyou-ssm1/dspsrsjmxribiao?name=" + params.name[0] + "&qdao=" + params.name[1] + "&time1=" + params.time1 + "&time2=" + params.time2,
        {
    ...
    //默认请求接口数据
    requestList = () => {
        axios.get('http://118.31.45.118:8888/lvyou-ssm1/dspsrsjmxribiao',
        {
            withCredentials: true
        }
        ).then((res) => {
    ...
    openOrderDetail = (params) => {
        window.open(`/#/common/order/detail3/：orderId3`, '_blank')
    }
    render() {
        const columns = [
            {
